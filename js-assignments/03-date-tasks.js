...
function timeSpanToString(startDate, endDate) {
    ...
    let millisecond = (endDate.getMilliseconds()-startDate.getMilliseconds()).toString();
    ...
    if(Number(second)<10 && Number(second)>=0) {
            second='0'+second;
        }    
     if(Number(millisecond)<10 && Number(millisecond)>=0) {
            millisecond='00'+millisecond;
        }else if(Number(millisecond)<100
...
function angleBetweenClockHands(date) {
    ...
    const radiansConvert = Math.PI/180;
    if(hours>11) {
        hours-=12;
    }
    const result = Math.abs(hours * 30 + minutes * 0.5 - minutes * 6);
    return (result <= 180 ? result : 360 - result) / 180 * Math.PI;
}
