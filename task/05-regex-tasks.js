...
function getRegexForIPv4() {
  const template = '(25[0-5]|2[0-4]\\d|[01]?\\d{1,2})';
  return new RegExp(`^${template}\\.${template}\\.${template}\\.${template}$`);
}
...
function getPasswordValidator(minLength) {
    ...
    return new RegExp(`^(?=${template}*[a-z])(?=${template}*[A-Z])(?=${template}*\\d)${template}{${minLength},}$`);
}
