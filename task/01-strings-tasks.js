...
function repeatString(value, count) {
    ...
    const basicLength = value.length;
    while(outputString.length < basicLength * count) {
        outputString += value;
    }
    return outputString;
}
...
function getRectangleString(width, height) {
    var top='',bottom='',middle='',horisontalLine='─',space=' ',verticalLine='│';
    if(width<2||height<2){
        return false;
    }
        top='┌'+horisontalLine.repeat(width-2)+'┐'+'\n';
        middle=verticalLine+space.repeat(width-2)+verticalLine+'\n';
        bottom='└'+horisontalLine.repeat(width-2)+'┘'+'\n';
...
function getCardId(value) {
    const pic = '♣♦♥♠';
    const num = 'A234567891JQK';
    return pic.indexOf(value.slice(-1))*13 + num.indexOf(value.slice(0,1));
}
