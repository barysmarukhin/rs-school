...
function reverseString(str) {
    return str.length > 1 ? str[str.length - 1] + reverseString(str.slice(0, -1)) : str;
}   
...
function isCreditCardNumber(ccn) {
    let sum = 0;
    ccn = ccn.toString();
    ccn.split('').map((item, i) => {
        if (ccn.length % 2 !== i % 2) {
            return sum += item * 1;
        }
        else {
            sum += item * 2 + (item > 4 ? -9 : 0);
        }
    });
...
function getDigitalRoot(num) {
    const array = num.toString().split('').map((item) => Number(item));
...
function timespanToHumanString(startDate, endDate) {
    ...
    if (difference <= 90 * seconds)
        return 'a minute ago';
    if (difference <= 45 * minutes)
        return `${Math.round((difference - 1) / minutes)} minutes ago`;
    if (difference <= 90 * minutes)
        return 'an hour ago';
    if (difference <= 22 * hours)
        return `${Math.round((difference - 1) / hours)} hours ago`;
    if (difference <= 36 * hours)
        return 'a day ago';
    if (difference <= 25 * days)
        return `${Math.round((difference - 1) / days)} days ago`;
    if (difference <= 45 * days)
        return 'a month ago';
    if (difference <= 345 * days)
        return `${Math.round(difference / 30 / days)} months ago`;
    if (difference <= 545 * days)
        return 'a year ago';
    return `${Math.round(difference / 365 / days)} years ago`;
}
...
function evaluateTicTacToePosition(position) {
    ...
    for (let row of winnerCoordinates) {
        let isWinner = true,
            winnerName = position[row[0][0]][row[0][1]];
        if (winnerName !== undefined) {
            for (let i = 1; i < row.length; i++){
                if(winnerName !== position[row[i][0]][row[i][1]]) {
                    isWinner = false;
                }
            }
            if (isWinner) {
